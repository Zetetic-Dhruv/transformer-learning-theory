...
| `quadRoute_alternations_le` · `seqChanges_blockRefine_le_two` · `quadDepthGrade_zero_ssubset_one` | `TemperedDesignLaw/QuadraticDepth` | **the self-attention depth bound + base rung** for quadratic-gated cascades (quadratic gate, affine branch). The per-layer order-2 switch (`λ₂` at arity 2: `≤ 2`) composes over depth via the new base-3 block-refine `seqChanges ≤ 3·u + 2`, giving the depth-`L` line bound `≤ 3^{L+1} − 1` at general `L` (the quadratic analogue of the affine `≤ 2^{L+1} − 1`). The base rung `quadDepthGrade 0 ⊊ quadDepthGrade 1` is a real `∉`: a depth-1 quadratic witness fires in the middle of a 3-point sample (alternates `2`), while depth-0 is the bare affine argmax (caps at `1`). The general-`L` depth witness needs an iterated ternary tent and is closed for the multi-expert router below |
| `quadDepthGrade3_ssubset_succ` · `ternTentRoute3_alternations_eq` · `quadRoute3_alternations_le` | `TemperedDesignLaw/QuadDepthSeparation3` | **the multi-expert self-attention depth hierarchy** (the genuine second-order depth separation): for a `k = 3`-expert quadratic router whose gate shares a common bilinear `QᵀK` form across experts (`Mcommon`) with per-expert linear prototypes, `quadDepthGrade3 L ⊊ quadDepthGrade3 (L+1)` for *every* `L`. The shared `Mcommon` cancels in expert-vs-expert comparisons, so the per-fiber gate is an affine nearest-anchor argmax with a tight base-3 ceiling `≤ 3^{L+1} − 1` (`quadRoute3_alternations_le`); an iterated ternary tent saturates `3^{L+1}` alternations on the triadic grid (`ternTentRoute3_alternations_eq`), exceeding it. This is the faithful `k`-expert attention router (shared QK, per-expert value), and the route folds genuinely three ways because the three branches are distinct |
| `affineArgmaxCell_FO` · `quadraticArgmaxCell_SO_semialgebraic` | `TemperedDesignLaw/FOSOMembership` | **the first-order / second-order membership placement.** The affine argmax cell is a first-order object: a finite intersection of affine half-spaces, each conjunct a genuine degree-1 inequality `{x | L x ≤ c}` with `L` linear (`affineArgmaxCell_conjunct_degree1`), convex and decidable by linear arithmetic. The quadratic (self-attention) argmax cell is second-order: a finite intersection of degree-2 polynomial sublevel sets (`quadraticArgmaxCell_conjunct_degree2`), semialgebraic and provably non-convex (`quadraticArgmaxCell_SO_not_convex`). A routing design's logical type is read off the score degree: affine routing is first-order verifiable, quadratic routing is second-order |
| `polyDepthGrade_ssubset_succ` · `polyTentRoute_alternations_eq` · `seqChanges_blockRefine_poly_le` | `TemperedDesignLaw/PolyDepthSeparation` | **the degree-`d` depth law, the master parameter.** For a degree-`d` polynomial nearest-anchor gate with `d+1` distinct branches, `polyDepthGrade d L ⊊ polyDepthGrade d (L+1)` for *every* `L` and *every* `d ≥ 1`. A degree-`d` score has at most `d` roots, so the gate folds `d+1` ways; the base-`(d+1)` block-refine `seqChanges ≤ n·u + (n−1)` gives the depth-`L` ceiling `(d+1)^{L+1} − 1`, and a `(d+1)`-ary tent saturates `(d+1)^{L+1}` alternations on the `(d+1)`-adic grid, exceeding it. This subsumes the affine (`d = 1`, `binCascadeGrade`) and multi-expert quadratic (`d = 2`, `quadDepthGrade3`) hierarchies as the `d = 1, 2` instances of one law, using `d+1` distinct anchors `scoreᵢ(s) = −((d+1)·s − (i + ½))²` for a genuine `(d+1)`-fold |
| `cubicArg_noABABA` · `CubicLines.pair_comparison_le` · `NoABABA` | `TemperedDesignLaw/CubicWidth` | **the cubic (degree-3) width structure** via order-3 Davenport-Schinzel. Each pairwise cubic difference has at most `3` roots, so the argmax of `n` cubics along an increasing sample makes at most `3` comparison flips per pair (`pair_comparison_le`), hence no `a,b,a,b,a` alternation (`cubicArg_noABABA`, the order-3 forbidden pattern `NoABABA`). The `≤ 3` bound is charged against root multiplicity rather than distinct roots: a tangency from above is a genuine double root (the touch point is a local minimum, so the derivative vanishes and `2 ≤ rootMultiplicity` in characteristic 0), and the flips dominate into `D.roots` as a multiset. The quadratic convexity / `OrdConnected` argument does not lift, since cubic sublevel sets need not be intervals, so degree 3 genuinely needs the multiplicity argument |

> **Scope.** Affine scores at carrier `ℝ` / `ℝᵈ` drive the hierarchy; the full self-attention (quadratic) regime is treated in `QuadraticExpressivity` / `QuadraticTameness` / `QuadraticWidth` / `QuadraticDepth` / `QuadDepthSeparation3` / `FOSOMembership` / `PolyDepthSeparation` / `CubicWidth`. All separations are real proved non-memberships, kernel-verified. The affine depth hierarchy holds at the general carrier `(Fin d → ℝ, Fin k)` for all `d ≥ 1`, `k ≥ 2`. Under full self-attention the convex power-diagram characterization is refuted, while the region count and finite-VC learnability both survive (polynomial scores are finite-dimensional). The general-`n` self-attention WIDTH separation is closed via the tight order-2 Davenport-Schinzel bound `λ₂(n) ≤ 2n − 1`; the depth-`L` line bound `≤ 3^{L+1} − 1` is proved at general `L`, with the base rung depth separation closed. Open and stated as such: the general-`L` depth separation (the depth-`L` bound + base rung are proved; an iterated-tent depth-`(L+1)` witness closes it for the multi-expert (`k = 3`, distinct-branch) router, `quadDepthGrade3_ssubset_succ`; the arity-2 binary case stays open because the gate's two outer intervals share a branch, so the route folds only two ways and closing it needs a parity-tracked alternation argument rather than the ternary tent), strict separation of the full any-arity grade with depth and arity varying together, and the full o-minimality-vs-tameness equivalence (the polynomial implies finite-dim implies tame direction is proved). The depth law is uniform in the score degree (`polyDepthGrade d L ⊊ polyDepthGrade d (L+1)` for every `L` and every `d ≥ 1`, subsuming the affine and quadratic cases), and the cubic width adds the order-3 Davenport-Schinzel structure (no `a,b,a,b,a`, at most 3 flips per pair); still open are the super-linear cubic region-count lower bound and the sharp inverse-Ackermann count `λ₃(n) = Θ(n·α(n))`, out of reach while `α` is absent from Mathlib.
