...
**What is the Lipschitz constant of self‑attention?** Kim, Papamakarios and Mnih proved standard dot‑product self‑attention "is not Lipschitz for unbounded input domain" [[Kim et al. 2021]](https://arxiv.org/abs/2006.04710); tight, certified constants remain open. *Traction here:* the kernel now carries a **constructive on‑domain constant** — single‑query attention moves by at most `2·nK·bV·(dB/scale)·(‖ΔQ‖+‖ΔK‖) + ‖ΔV‖` on `‖Q‖,‖K‖ ≤ B` (`attnOut_lipschitz_on_ball`), with the non‑Lipschitzness isolated entirely to the bilinear score map and the softmax mixing globally Lipschitz — and it turns the Kim et al. boundary into a *layer instance*: although self‑attention has no global constant, it is a genuine certificate‑side `ExecLayer` over the metric subtype `↥(closedBall 0 B)` (`selfAttnExecLayer`, via `execLayerOfForwardInvariant`), the bounded‑input cap carried by the type. And the constant is not free‑floating: it now carries a **full certified generalization bound** for an attention model whose learnable value projection is genuinely in the capacity (`attnHead_certified_generalization`) — composed against TorchLean's *literal* attention operation, which that model provably equals in coordinates (`matCoords_scaledDotProductAttention`). The Kim et al. boundary has become a hypothesis inside a machine‑checked learning bound, not an obstacle to one.
...
| `attnHead_certified_generalization` | `Bridge/AttentionTransformerCertificate` | **the bound, instantiated end‑to‑end on a concrete attention model** — a dot‑product head with a *learnable value projection* `W`, every capstone hypothesis discharged from the head's structure; the learnable weight is an attention weight, so the capacity term measures the attention class |
| `matCoords_scaledDotProductAttention` | `Bridge/AttentionSpecBridge` | **the literal binding** — that certified head *is* TorchLean's `Spec.scaledDotProductAttention`, read in coordinates, via the softmax‑stabilization bridge `vecGet_softmaxVecSpec` and the `get2`‑coordinate reads |
...
The bound is no longer merely assembled — it is **instantiated**, on a model proven to be the real operation. `attnHead_certified_generalization` discharges the capstone for a genuine attention head; `matCoords_scaledDotProductAttention` shows that head is TorchLean's literal `scaledDotProductAttention` in real arithmetic. Two things remain, both breadth rather than depth: the full multi‑block `Spec.Transformer` (the composition `paramComp_param_lipschitz_on'` is already depth‑uniform, and the bounded‑activation roster — `clampCoord`, `layerNormExecLayer`, `layerNormCoord_norm_le` — supplies the per‑op atoms), and the IEEE32 executed‑attention rounding envelope that carries the binding from ℝ to the float32 op.
...
- **Machine‑checked:** everything in *Foundations* above. The headline results (`transformerForwardMap_executed_measurable`, `executed_risk_transfer`, `get2_layerNorm`, `fp32FoldlErrorBudget_closed_form`, `ie32_foldl_closed_envelope`, `execComp_envelope`/`execComp_risk_transfer`, the generalization capstone `certified_executed_generalization_dudley`, the Lipschitz constants `layerNormCoord_lipschitz` / `attnOut_lipschitz_on_ball` / `selfAttnExecLayer`, the attention‑model certificate `attnHead_certified_generalization`, and the literal binding `matCoords_scaledDotProductAttention`) reduce to only `propext`, `Classical.choice`, `Quot.sound` — no `sorry`, no added axioms.
...
- **Done since the last pass:** the generalization capstone is now **instantiated** on a concrete attention model with a learnable attention weight in the capacity (`attnHead_certified_generalization`), and that model is proven to be TorchLean's *literal* `scaledDotProductAttention` in real arithmetic (`matCoords_scaledDotProductAttention`) — the softmax‑stabilization coordinate bridge and the bounded‑activation roster (`paramComp_param_lipschitz_on'`, `clampCoord`, `layerNormExecLayer`) are all in place.
- **In progress:** two extensions of breadth. (i) The full multi‑block `Spec.Transformer`: the composition `paramComp_param_lipschitz_on'` is already depth‑uniform and the per‑op atoms exist, so this is assembly over one bounded activation domain (re‑established each block by layer‑norm). (ii) The IEEE32 executed‑attention rounding envelope that carries the literal binding from ℝ to the float32 op (`ExecLayer.exec` = the IEEE32 attention, `rnd` = its ULP bound) — the α‑bridge for attention, parallel to the existing linear/ReLU `SpecExecLayer`s.
