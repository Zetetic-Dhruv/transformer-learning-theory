...
> **Scope.** Affine scores, the cross‑attention / parameter‑key regime, at carrier `ℝ` / `ℝᵈ`. Both separations are real proved non‑memberships, kernel‑verified, with witnesses whose alternation counts are computed point‑by‑point. Two extensions are open and stated as such: the semialgebraic generalization for the quadratic scores of full self‑attention (the region‑count argument survives, the convexity corollary does not), and strict separation of the full any‑arity grade beyond the fixed‑width and fixed‑depth slices.
