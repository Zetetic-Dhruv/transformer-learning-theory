...
| M6 | Least-survivor = Occam optimum = softmax top-1 at every temperature; tempering degrees of freedom are the weight stratum, unidentifiable from the hard channel. | CHART-FIXING (⊤-σ-algebra on the soft face); tempering identifiability banked at the score grain | witnessed-exactly (hard face); witnessed-at-instance (soft face); witnessed-at-instance (tempering, score grain: `scoreVector_not_maskDetermined`) | `StoreCharacterization.selection`; `abduce_eq_some_get_min'`; `gibbs_top1_eq_abduce`; `abduce_eq_fitRouter_route`; `min'_unique_argmax_of_strictAnti` |
