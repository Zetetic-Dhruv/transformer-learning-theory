...
operation level, a single decidable bit (no `exp`/`log`/`div`, zero transcendental operations). The
same least-maximizer selection, run on a `Bool` comparison matrix, is an explicit **computable**
algorithm `argmaxFromCmp`.

`argmaxFromCmp` is a plain `def` (it `#eval`s), in deliberate contrast to the `noncomputable`
`leastArgmax`; the computability gap is exactly the `ℝ`-`≤` versus `Bool`-comparison gap. The
content is the (non-defeq) factorization
`leastArgmax v hk = argmaxFromCmp (fun i j => decide (v i ≤ v j)) hk`, which
...
/-- **The factorization: the real-score route equals the comparison-only program.** The `noncomputable`
least-argmax of real scores equals `argmaxFromCmp` applied to their Boolean `≤`-comparison matrix.
The routing decision, defined over `ℝ` where `≤` is not computably decidable, coincides with an
explicit transcendental-free **decidable** algorithm.

The proof is a uniqueness argument (`isLeastArgmax_unique` against `leastArgmax_spec`), not
definitional: `leastArgmax` is `Finset.min'` of a filter over the reals while `argmaxFromCmp` is
`Fin.find` over `Bool`, so the two are not defeq and the equation does not close by `rfl`. -/
...
comparisons `score i ≤ score j`. The symbol/hard route uses score comparisons only, with zero
transcendental operations. This is `leastArgmax_eq_argmaxFromCmp` at `v := A.score ρ x`
(`A.route` is definitionally `leastArgmax` of the scores). -/
...
comparison bits, never the real magnitudes, and is computed with **zero transcendental operations**
(no `exp`/`log`/`div`).
...
fails it (`comparisonOnly_not_vacuous`). The soft/mixture route sits on the failing side, since its
weights read exact magnitudes and are never one-hot (`softWeights_lt_one`, `ExactnessImpossibility`). -/
def ComparisonOnly (f : (Fin k → ℝ) → Fin k) : Prop :=
  ∃ g : (Fin k → Fin k → Bool) → Fin k, ∀ v, f v = g (fun i j => decide (v i ≤ v j))

/-- **The hard route is comparison-only.** The least-argmax routing map factors through the Boolean
comparison matrix, witnessed constructively by `argmaxFromCmp` (the decoder) via
`leastArgmax_eq_argmaxFromCmp`. The symbol channel's decision is a function of score comparisons
alone. -/
...
decoder can reproduce it. This confines the predicate to the order-determined maps, exactly where the
...
The operation-type results here live at the real-arithmetic level. Their *executed* refinement
(that the comparison-only route run in IEEE binary32 equals the ideal-ℝ route with **zero** rounding
error, `b = 0`) is proved in `TLT_Proofs.Routing.SymbolOpcountExecuted`
...
atom is *real-exact* (`compare_eq_some_lt_iff_toReal_lt` in `NN.Floats.IEEEExec.BridgeFP32` shows the
...
(`neural-networks` / TorchLean `IEEE32Exec`), which is imported only by the sibling module.

Over `ℝ`, `decide (v i ≤ v j)` is `Classical.dec` and **noncomputable**; genuine *executable*
comparison-only exactness appears only at the fp32 atom. -/
