...
`≤`-comparison matrix. This file records the **dual** capacity reading of that same comparison object:
each per-pair comparison is a **halfspace** — the sign of the score difference — so, when the scores
...
* `comparisonClass_vcDim_le` — **conditional Dudley/VC bound (the capacity face).** If the per-pair
  ...
  hypothesis is load-bearing. Non-strict `signClassLE` (`0 ≤ g`), not strict `signClass` (`0 < g`): the
  tie `sᵢ = sⱼ` is the one the least-index argmax breaks.

This is the capacity face dual to the opcount factorization: one comparison object, read two ways —
...
concept `0 ≤ sⱼ − sᵢ` of the score-difference functional `x ↦ sⱼ(x) − sᵢ(x)`. (In-layer; `sub_nonneg`.
...
bound is false, so the hypothesis is load-bearing — it is exactly the linearity of the router's
