...
theorem cascadeBadEvent_measurableSet_iff (M : MoECascadeCode β width) (L : ℕ) :
...
theorem cascadeBadEvent_measurable_of_sigmaCompact [SigmaCompactSpace β]
    (M : MoECascadeCode β width) (L : ℕ) :
    MeasurableSet (cascadeBadEvent M L) :=
  (cascadeBadEvent_measurableSet_iff M L).mpr
    (measurableSet_range_of_continuous_of_sigmaCompact M.g_cont)
