...
This file reconciles the two: at the canonical single-head configuration (one head, identity
query/key/value/output projections), the multi-head op reduces — in coordinates — to the single
scaled-dot-product attention already bound by `matCoords_scaledDotProductAttention`, hence to
`selfAttn`.
...
- Provenance: Vendored-glue (MHA h=1 ↔ selfAttn); a few dependently-typed tensor/Fin-cast
  plumbing lemmas (`get2_ext`, `vecTensor_ext`, `vecGet_eqRec`) are TLT infrastructure.
-/
...
read `get2` are equal — the bridge that lifts a coordinate identity (`matCoords … = …`) to a tensor
...
tensor-valued companion of `matCoords_matMulSpec`, obtained by `get2`-extensionality — so the literal
...
TorchLean as "the core of `nn.MultiheadAttention` / `TransformerEncoderLayer`"; this binds that core
to the model the certified bound measures, with no head-reshape. The identity projections collapse via
`matMulSpec_id_right`; the inner attention is the already-bound `scaledDotProductAttention`. -/
...
equality to a flat-vector equality — no `unflattenSpec` coordinate read required. -/
...
reindex — so `1*d`-vs-`d` non-definitional-equality never touches a *tensor*, only an index. -/
...
equal — lifts a flat-vector coordinate identity to a tensor identity. -/
...
`splitHeadsSpec I 1 d` recovers `I` reshaped to the per-head `(n, d)` — the key fact that reduces the
...
actually calls) reduces — through `splitHeadsSpec_one`, `combineHeadsSpec_one`, and the identity-projection
collapse `matMulSpec_id_right` — to a reshape of `scaledDotProductAttention` applied to the per-head view
of the input. Composed with `matCoords_scaledDotProductAttention`, this binds the literal multi-head op to
the certified `selfAttn`, with no reshape pathology surviving. -/
...
/-! ### Toward #2 — general `numHeads`: the swap is a real transpose -/
