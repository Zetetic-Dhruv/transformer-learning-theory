...
/-- The literal `maximum x 0` is finite when `x` is finite, so the ReLU layer's output feeds
...
A re-export of `toReal_sqrt_eq_fp32Round_of_isFinite` in the shape the stack-side forward error consumes. -/
