...
relative `u`-bound). These are the no-underflow/no-overflow hypotheses for the reduction. -/
...
The softmax-denominator non-underflow is the sharp boundary: a sum of positive exponentials can
underflow to a non-normal or zero float, after which no clamp-ball argument recovers the quotient.
This is surfaced as the explicit hypothesis `Dlo ≤ D̃` (a positive lower bound on the rounded
denominator). The remaining conditions (the exp bounds, the denominator fold, the quotient and mix
rounding) are derived from the round-to-nearest model. -/
...
/-- The bundled normal/finiteness hypotheses for the executed head on input `Y` (clamped to `X`): the
no-underflow/no-overflow conditions on the run-time intermediates of the binary32 forward.
`hDlo`/`hDlo0` carry the softmax-denominator non-underflow assumption. -/
...
/-- **Certified float32 generalization bound for the executed attention head, with the rounding
envelope derived.** Identical in conclusion to `attnHead_executed_certified_generalization`, but the
executed map is the concrete literal-fp32 head `execAttnStar` and the rounding bound is the derived
closed form `rndStar n d B Λ scale Dlo`, with the envelope hypothesis discharged by
`attnForwardError`. The additional inputs are operating-range data: the weight `ℓ¹`-bound `Λ` (with
`0 ≤ Λ`, `hW`), the positive scale `hscale`, the roundoff conditions `n·u < 1`, `d·u < 1`, and the
bundled binary32 normal/no-overflow/softmax-denominator-no-underflow conditions `hnormAll` (with
positive denominator floor `Dlo`). The rounding correction `2·Lℓ·rndStar` is a closed form in the
model's parameters. -/
