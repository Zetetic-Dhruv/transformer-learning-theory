...
/-!
# Bit-level `Exec32.exp` error: the fixed-point rounding atom (L1)
...
evaluate `2^f` by a degree-10 fixed-point Taylor polynomial, then round `2^k · 2^f` to binary32. The
error `|toReal(Exec32.exp x) − Real.exp(toReal x)|` is derived from this algorithm.

Every fixed-point step rounds an integer quotient to nearest, ties-to-even, via `roundQuotEvenInt`.
The L1 rounding atom is that the ties-to-even integer round is within half a unit of the exact
quotient: the building block for both the range reduction and the Horner polynomial evaluation
(`fixedMul = roundDivPow2EvenInt (· * ·) 48 = roundQuotEvenInt (· * ·) 2⁴⁸`).
...
/-!
## References
- [46] table-driven exp (range-reduce/polynomial/reconstruct); [47] argument reduction; [43] §5.1
  Horner backward error; [44] ½-ulp; `Real.exp_bound` (Taylor/Lagrange remainder); Mathlib `ln 2`
  bracket; `abs_pow_sub_pow_le`.
-/
...
With `s = t·ln 2`, `|t| ≤ ½`, this bounds the approximation error of the exact Taylor polynomial of `2ᵗ`
that `Exec32.exp` evaluates in fixed point. This is the `E₁` term of the L3 decomposition. -/
...
interval checks against `ln 2` with one structural lemma. The remaining `|cₘ/2⁴⁸ − Lᵐ/m!|` is then an
...
`≤ 10⁻⁹` (actual value `≈ 2.8×10⁻¹¹`). No irrational `ln 2` enters this half of E₂; it closes by
`norm_num`. -/
...
with the Horner-rounding bound `evalExp2Poly_approx_codePoly` (E₃). -/
