...
- Provenance: Innovation — the machine-checked bit-level error certificate (10⁻⁶) for TorchLean's
  exact degree-10 fixed-point `2ᵗ` evaluator; constituent models classical.
- TLT contribution (Dhruv Gupta), `evalExp2Poly_error`: a machine-checked 10⁻⁶ bit-level error certificate for TorchLean's exact degree-10 fixed-point 2ᵗ Taylor evaluator. Method: triangle decomposition into the Taylor remainder, an 11-coefficient base-perturbation orbit (abs_pow_sub_pow_le), and the integer-Horner fold rounding recurrence, with a rational-anchor δ-routing of ln 2. (The rounding model itself is classical.)
-/
