...
and std as the `Vexec` uniform-`1/d` matmul reductions (the bit-level fp32 reductions, read over
`ℝ`), and proves the carriers' `hln` with the closed-form `lnBudgetVal d B Cγ Maff`. Every per-op
rounding (`ρm` mean, `ρs` std, `ρround` affine) is the floor-driven closed budget. The LayerNorm is
a faithful executed ℝ-model (its mean/std are `toReal` of the IEEE reductions), with no abstract budget left.
-/
...
executed centered squares, clamped, regularised, and square-rooted. The bit-level executed std. -/
...
`layerNormCoord`, under the normal-range regime (the two `Vexec` reductions normal, the squaring
