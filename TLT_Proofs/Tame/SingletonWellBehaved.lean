...
`U` field. The **Borel** strengthening for `singletonClassOn A` under `MeasurableSet A`
generalizes `Tame.singletonBadEvent_measurable` from `m = 1, c = zeroConcept` to every sample
size `m` and every measurable target `c`, via a finite-sample reduction of the uncountable
`∃ h ∈ singletonClassOn A` to the `2m` sample-matching singletons plus the `zeroConcept` residual.
-/

/-!
## References
- [7] Def. 3.2 well-behavedness (V/U maps); [12] the singleton class; [9][10] permissibility /
  image-admissible-Suslin (bypassed here by an explicit finite reduction); [57] FLT
...
well-behavedness, established here at the strict **Borel** level (`MeasurableSet`), which
