...
/-!
# The tempered design-law certificate with real S5 statistics

This module replaces the placeholder statistical bound `0 ≤ 1` from
`CertificateAssembly.temperedDesignLawCertificate_concrete` with the S5 expected-gap bound
`symbolGap ≤ symbolBound`, where

* `symbolGap := (∫⁻ ofReal G).toReal` is the real expectation of the single-concept positive-part
  generalization-gap functional `G` (the positive part of the unbiased zero-mean single-concept
  gap), and
* `symbolBound := √(2 log 2 / m) + (∏ Sauer)·(2/√m)·√(2π)` is the Sauer–`√(log/m)` envelope.

## The measurable-target path

The S5 theorem `temperedSymbol_expectedGap_hard_le` requires `WellBehavedVC X (routeLossClass …)`,
the NullMeasurable double-sample regularity quantified over all concepts `c`. Reducing that to a
continuous carrier is the standing open problem `WellBehavedVC_automatic`
(`FLT_Proofs/Complexity/Measurability.lean`), which is not addressed here. Instead the S5 chain is
mirrored with the measurable-target variant `WellBehavedVCMeasTarget` (the same event, quantified
only over measurable targets):

* `boolClass_gen_gap_measTarget` mirrors `boolClass_gen_gap`, with the regularity leg applied at the
  measurable target `c` via `hWB D c hc_meas m ε`.
...
The `WellBehavedVCMeasTarget` hypothesis is then proved for the route-loss class via the
Borel-analytic bridge `borel_param_wellBehavedVCMeasTarget`: the route-loss concept evaluation is
jointly measurable in `(ρ, x)` (`routeLossConcept_jointMeasurable`), and `A.Ρ` is a standard Borel
space with `X = Fin d → ℝ` Polish/Borel.

## The concrete carrier (the attention router)

The statistical carrier is the literal scaled-dot-product attention router
`Bridge.attentionScoreRouter`. Its score is `⟨x, Kᵢ⟩ = ∑ₗ xₗ·Kᵢₗ` (`attnScore_eq_sum`), so the
score differences `x ↦ ⟨x, Kⱼ − Kᵢ⟩` are linear in `x`, lying in the `d`-dimensional span of
coordinate functionals `coordSpan d` (`attnScoreDiff_mem_coordSpan`). This is the finite-dimensional
`W` needed by the arrangement-VC Sauer product.
-/
...
`temperedSymbol_expectedGap_hard_le`, with the single change that the NullMeasurable double-sample
regularity leg uses the measurable-target predicate `WellBehavedVCMeasTarget`, applied at the
(measurable) target `c`. The source theorems are not modified. -/
...
`(ρ, x) ↦ (route ρ x, y x)`, via `route_measurable` (joint `Ρ × X`) paired with `y ∘ snd`. This is the
...
/-- **The route-loss class satisfies `WellBehavedVCMeasTarget`.** The route-loss class is
`Set.range (fun ρ => routeLossConcept A hk ρ y)`, a Borel-parameterized class (parameter space `A.Ρ`
is standard Borel; the evaluation is jointly measurable by `routeLossConcept_jointMeasurable`). The
Borel-analytic bridge `borel_param_wellBehavedVCMeasTarget` then yields the measurable-target
NullMeasurable regularity without requiring the stronger `WellBehavedVC`. -/
...
arrangement Sauer–Shelah product via `routeLossClass_growthFunction_le`, exactly as in
`symbolRoute_gen_gap`. -/
...
generalization gap `∫⁻ ofReal G` is bounded by the Sauer–`√(log/m)` envelope, with the tail input
supplied by `symbolRoute_gen_gap_measTarget` and the regularity carried as `WellBehavedVCMeasTarget`. -/
...
/-! ## 4. The finite-dimensional `W` for the attention score differences (linear case) -/
...
/-! ## 5. The single-concept gap functional `G` and its three properties -/
...
true-minus-empirical gap against `c`. The single-concept gap is an unbiased zero-mean estimator, so
its positive part has positive expectation in the presence of sampling variance. -/
...
either `ε ≤ 0` (in which case the target `c = routeLossConcept … ρ₁ y ∈ routeLossClass` has gap
exactly `0 ≥ ε`, by `trueErrorReal_self_zero` and `empiricalError_self_zero`) or `ε ≤` the
`h₀`-gap, witnessed by `h₀`. -/
...
/-! ## 6. The S5 statistical bound on the attention carrier

The bound `(∫⁻ ofReal G).toReal ≤ √(2 log 2 / m) + (∏ Sauer)·(2/√m)·√(2π)` on the literal
scaled-dot-product attention router. The `WellBehavedVCMeasTarget` regularity is proved (not carried)
via the Borel-analytic bridge; the finite-dimensional `W` is `coordSpan d`. -/

/-- **The real S5 statistical bound on the literal attention router.** The carrier is
`Bridge.attentionScoreRouter d nK` on `X = Fin d → ℝ`; its linear score differences give `W =
coordSpan d`, the regularity is `routeLossClass_wellBehavedVCMeasTarget`, and the gap functional is
`singleConceptGap`. The real expected gap is bounded by the Sauer–`√(log/m)` envelope. -/
theorem attentionRoute_realStatBound
  ...
  -- the regularity and the target concept `c = routeLossConcept … ρ1 y`
...
/-! ## 7. The assembled certificate with real S5 statistics

Feeds the real bound `symbolGap ≤ symbolBound` from `attentionRoute_realStatBound` into
`temperedDesignLawCertificate` as the `stat_cert` leg. The other carried inputs (the abstract gap with
`smooth_cert`/`hard_cert`, the classical non-Borel base range) are the same as in
`temperedDesignLawCertificate_concrete`. The statistical carrier is the literal attention router
(the same family as the hard-tame leg). -/

/-- **The Sauer envelope is strictly positive** for a finite sample size `m > 0`: the leading
`√(2 log 2 / m)` term is positive (since `log 2 > 0`) and the Sauer remainder is nonnegative. This
discharges the certificate's `symbolBound_pos` field on the real statistical leg, so the carried bound
is genuinely positive and `symbolGap ≤ symbolBound` is never the trivial `0 ≤ 0`. -/
theorem sauerEnvelope_pos (d nK m : ℕ) (hm : 0 < m) :
    0 < Real.sqrt (2 * Real.log 2 / m)
      + (↑(∏ _p : Fin nK × Fin nK,
            ∑ r ∈ Finset.range (finrank ℝ (coordSpan d) + 1), (2 * m).choose r) : ℝ)
          * (2 / Real.sqrt m) * Real.sqrt (2 * Real.pi) := by
  have hlog : 0 < Real.log 2 := Real.log_pos (by norm_num)
  have hm' : (0 : ℝ) < (m : ℝ) := by exact_mod_cast hm
  have h1 : 0 < Real.sqrt (2 * Real.log 2 / m) :=
    Real.sqrt_pos.mpr (div_pos (by linarith) hm')
  have h2 : 0 ≤ (↑(∏ _p : Fin nK × Fin nK,
            ∑ r ∈ Finset.range (finrank ℝ (coordSpan d) + 1), (2 * m).choose r) : ℝ)
          * (2 / Real.sqrt m) * Real.sqrt (2 * Real.pi) := by positivity
  linarith

/-- **The TD16 certificate with real S5 statistics.** An inhabitant of
`TemperedDesignLawCertificate` whose hard-tame statistical leg carries the S5 expected-gap
inequality `(∫⁻ ofReal G).toReal ≤ Sauer–√(log/m) envelope` (from `attentionRoute_realStatBound`).
The carrier is the literal scaled-dot-product attention router on `Fin d → ℝ`. The abstract gap
`gap := 0` (satisfying both `smooth_cert`/`hard_cert` by `gapZero_satisfies_certs`), the expressivity
ladder `E` (whose strict depth separation is the type-forcing teeth; the binary-carrier witness is
`MuxCertificate.binaryExpressivityLadder`), and the classical non-Borel base range are carried as in
`temperedDesignLawCertificate_concrete`; every other field is proved from a named theorem. -/
def temperedDesignLawCertificate_real
  ...
  temperedDesignLawCertificate d nK hk ρ M Ldepth (0 : Measure GhostPairs1)
    ...
    -- the S5 statistics, bundled: gap := (∫ G).toReal, bound := the Sauer envelope (positive for m > 0)
    { gap := (∫⁻ xs, ENNReal.ofReal
              (singleConceptGap (Bridge.attentionScoreRouter d nK) hk y ρ0 ρ1 D m xs)
            ∂(Measure.pi fun _ : Fin m => D)).toReal
      bound := Real.sqrt (2 * Real.log 2 / m)
        + (↑(∏ _p : Fin nK × Fin nK,
              ∑ r ∈ Finset.range (finrank ℝ (coordSpan d) + 1), (2 * m).choose r) : ℝ)
            * (2 / Real.sqrt m) * Real.sqrt (2 * Real.pi)
      gap_nonneg := ENNReal.toReal_nonneg
      bound_pos := sauerEnvelope_pos d nK m hm
      certified := attentionRoute_realStatBound d nK hk y hy ρ0 ρ1 D m hm }
    E
    hwild_nonBorel
