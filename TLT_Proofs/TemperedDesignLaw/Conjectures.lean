...
corners (ideal/float × soft/hard). The soft→hard bridge is the *same* grammar as the real→float bridge:
...
the weights are uniform and this fails — the third, score-decoupled regime; see GB1.) -/
...
/-- **The margin function.** Top score minus second score under the `leastArgmax` order — the gap that
the route survives perturbation by (well-defined by `isLeastArgmax_unique`). -/
...
region — both the `β`-shell complement and the `u`-shell complement live here at their radii). -/
...
β-shell at a different radius, here the float-margin band where the executed route may differ. -/
...
`(k−1)·exp(−β·γ)`: softmax's structural privilege is margin-linear log-leakage. -/
